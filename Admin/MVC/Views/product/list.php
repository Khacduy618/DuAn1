<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="m-0 font-weight-bold">Products Management</h2>
    </div>
        ...
        <div class="col-md-2">
            <select name="category" id="category" class="form-control" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
                ...
                <?php foreach($categories as $category): ?>
                    <option value="?mod=product&act=list&product_cat=<?= $category['category_id'] ?? 0 ?>&status=<?= $status ?>&per_page=<?= $_GET['per_page'] ?? 12 ?>&page=<?= $_GET['page'] ?? 1 ?>&field=<?= $_GET['field'] ?? '' ?>&sort=<?= $_GET['sort'] ?? '' ?>"
                        <?= (isset($_GET['product_cat']) && $_GET['product_cat'] == $category['category_id']) ? 'selected' : '' ?>>
            ...
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <img src="<?= BASE_URL ?>/uploaded/<?= $product['product_img'] ?>" 
                                class="img-thumbnail" 
